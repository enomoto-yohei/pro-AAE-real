        ...
        for batch in poseloader:
            ...
            for i in range(len(z)): #100回
                for n in range(128):  #128回
                    ...
                    save_name = save_name.replace("/home/milab/Desktop/10_13/10_13_実環境追加分/after_renama","")#ここが今から潜在変数にしたい画像のフォルダ名
                    with open(args.name+ "/10_14_hyou/jitukan_2/" +save_name+".txt","a") as f:#ここが潜在変数の保存するフォルダ名※フォルダ名の前に実行コマンドで指定したもののあと
                        f.write(latent_variable + "\n")


            #AE_sum += z* batchsz[python3 pose_encoder2h.py --load ./AAEt/ckpt/aae_0000002000.mdl --pose ./Z/imakarasenzaika/ --name Z
